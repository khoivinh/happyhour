...
## Current state
- 2026-07-15: **onboarding tagline** — "Welcome to the indispensable world clock." (Zalando Black 36/38, −1px, 60% width, left-aligned to Add Time Zone) shows only for genuinely new visitors, gated on a persistent `world-happyhour-onboarded` flag (returning users with saved zones are silently onboarded; seeded tiles stay). First-ever add fades it and slides the button + grid up via a CSS grid `1fr→0fr` row collapse (no anim lib), respecting `prefers-reduced-motion`. All in `time-zone-converter.tsx`; 25 Playwright assertions pass. **Open:** mobile 375px breaks the 36px/60% headline into ~5 lines — flagged for Khoi. Figma `338:3813`.
...
## Next steps
- Decide the mobile rendering of the onboarding tagline (36px/60% → ~5 lines at 375px)
